...
**M6 混淆定位（已实装）**
- 动机：M4 的"最弱音素点名"只回答了"该练哪"，用户更想知道"发成了
  什么"（已知风险一节预言音素级后处理比算法本身更花功夫——这就是
  第一步）。
- 落法：`PhoneGopResult::confused_with`。score_word 对每个音素的
  强制区间逐类平均证据取 argmax（排除 blank——静音不是"发成了
  什么"），明显压过记分证据（log 域 > 0.7，约 2 倍概率）且能映射
  回不同 ARPAbet 时记入。合写展开空格连接（ɑːɹ → "AA R"）。
- 四道不报门槛：主键/容忍变体天然过不了 margin（均值 ≤ best_mean，
  地道闪音不会被报"发成了 ɾ"）；映射回同一 ARPAbet 的自由变体
  （ASCII g 对主键 ɡ）不算错；词表外多语符号（法/德元音）映射不回
  ARPAbet，报用户看不懂的符号没有意义；margin 不足（相邻类抖动
  量级 0.1-0.3）不报——高分音素天然安静。
- 展示：GUI 逐音素行内箭头（"T 0.13→ER"）+ 最弱音素点名带"听起来
  像 X"；CLI --pron-score 行尾 `-> ER`。
- 测试教训（第二次踩）：合成数据里主键全帧无证时，Viterbi 落点是
  平局任意选——T 可能被排到 ʌ 帧上，混淆跟着报错对象。主键给弱
  证据（-2，真模型形态）落点才确定。与 flap 测试同坑，已注释互链。
